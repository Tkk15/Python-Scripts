...
def SaveLineToFile(line): todays_file = open(file_name, 'a') #open todays file (append mode)
...
def OnKeyboardEvent(event):
    ...
    if(window_name != event.WindowName): #if typing in new window
        ...
        else: line_buffer += chr(event.Ascii) #add pressed character to line buffer
